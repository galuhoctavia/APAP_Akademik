...
//            System.out.println(listIdMatkul.get(allKelas.get(i).getId_matkul()).getListKelas().size());
        }
        for(String key : listIdMatkul.keySet()){
            listMatkul.add(listIdMatkul.get(key));
//            System.out.println(listIdMatkul.get(key).getListKelas().size());
        }

//        model.addAttribute("allKey", allKey);
    ...
    public String addIrsSubmit(Model model, @RequestParam("result") List<String> result){
        ...
        for (int i = 0; i<result.size(); i++){
            AddIrsSpec temp = new AddIrsSpec(mhs, result.get(i), idKelas);
    ...
    public String viewStatusKelulusan(Model model){
        ...
        if(totalSKSLulus >= syaratSKSLulus) {
        	cekLulus = true;
        	cekSisa = totalSKSLulus - syaratSKSLulus;
        } else {
        	cekLulus = false;
...
//        System.out.println(syaratLulus);
...
//        return "login";
    ...
    public String dropIrs(Model model){
        ...
        if(auth.getName() != null){
            ...
            MahasiswaEntity mhs = mhsService.getMahasiswaByUsername(name);
            List<IrsEntity> listIrs = irsService.getIrsByUsernameAndTerm(mhs.getUsername(),"2014/2015-2"); //trial, term is not set
            model.addAttribute("listIrs",listIrs);
    ...
    public String dropIrsSubmit (Model model, @RequestParam(value = "result", required = false) List<Integer> result){
        System.out.println(result.size());
        for (int i = 0; i<result.size(); i++){
            irsService.deleteIrsById(result.get(i));
        }
        model.addAttribute("result", result);
...
/*
    ...
    public String indexSubmit(Model model,
                                     @ModelAttribute AddMahasiswaSpec mhsSpec){
        System.out.println("masuk");
        String username = mhsSpec.username;
        AkunEntity akun = akunService.getAkunByUsername(username);
        if (mhsSpec.password.equals(akun.getPassword())) {
            model.addAttribute("username",username);
            if (mhsSpec.isAdmin == 1) {
